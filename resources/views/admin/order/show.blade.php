  ...
  <div class="box-header with-border">
    ...
    <div class="box-tools">
      <div class="btn-group pull-right" style="margin-bottom: 10px">
        <a href="{{route('order.index')}}" class="btn btn-sm btn-default"><i class="fa fa-list"></i> {{__('shop.Back to list')}}</a>
      </div>
    </div>
  </div>
  <br>
    ...
    <table class="table table-bordered ">
      ...
      <tr>
        <td>
          ...
          @if ($item->refund == 'Y')
          <span style="margin-left: 30%;color:red" >{{__('shop.RefundStatus')}} {{__('shop.quantity')}}:{{$item->refundQuantity}}</span>
          @endif
        </td>
      ...
      <tr>
        <td colspan="4">
          <a class="pull-right" href="{{route('order.sand',['id' => $order->id ])}}"><button class="btn btn-success">{{__('shop.Send')}}</button></a> 
        </td>
      </tr>
      @else
      <tr>
          ...
          <tr>
            ...
            <td>
              ...
              <form style="display: inline-block" id='refund-disagree' action="{{route('order.refundDisagree')}}" method="post">
                {!! csrf_field() !!}
                <button class="btn btn-sm btn-danger " id="btn-refund-disagree" type="button" value="" onClick="subForm()";>{{__('shop.Disagree')}}</button>
