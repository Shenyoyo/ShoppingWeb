...
<ul class="nav nav-pills">
    ...
    <li class={{($id == $category->id) ?'active' : ''}}>
        <a href="{{ route('shop.category', ['id' => $category->id ,'orderby' => 'asc']) }}">{{ $category->name }}</a>
    </li>
    @endforeach
        ...
        <div class="thumbnail">
            <div class="caption text-center">
                <a href="{{ route('shop.show', [$product->id]) }}"><img
                        {{-- src="{{asset('storage/'.$product->file->filename.'')}}" alt="product" --}}
                        src="{{asset('uploads/'.$product->image.'')}}" alt="product"
...
<div  class="alert alert-danger text-center">
    <div>{{__('shop.data not found')}}</div>
</div>
